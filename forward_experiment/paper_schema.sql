...
-- paper_forward_execution_evidence
CREATE TABLE paper_forward_execution_evidence(run_id VARCHAR PRIMARY KEY, captured_at_utc TIMESTAMP WITH TIME ZONE NOT NULL, diagnostics JSON NOT NULL);;
...
-- paper_run_release_provenance
CREATE TABLE paper_run_release_provenance(run_id VARCHAR PRIMARY KEY, git_commit VARCHAR NOT NULL, git_dirty BOOLEAN NOT NULL, hardening_manifest_sha256 VARCHAR NOT NULL, execution_protocol_version VARCHAR NOT NULL, captured_at_utc TIMESTAMP WITH TIME ZONE NOT NULL);;

-- paper_market_rule_evidence (prospective Binance MARKET-rule audit evidence)
CREATE TABLE paper_market_rule_evidence(run_id VARCHAR NOT NULL, symbol VARCHAR NOT NULL, contract_version VARCHAR NOT NULL, reference_price_decimal VARCHAR, reference_price_source VARCHAR NOT NULL, reference_price_timestamp_utc TIMESTAMP WITH TIME ZONE, lot_min_qty VARCHAR, lot_max_qty VARCHAR, lot_step_size VARCHAR, market_lot_min_qty VARCHAR, market_lot_max_qty VARCHAR, market_lot_step_size VARCHAR, min_notional VARCHAR, min_notional_applies_to_market BOOLEAN NOT NULL, min_notional_avg_price_mins INTEGER NOT NULL, notional_min VARCHAR, notional_max VARCHAR, notional_min_applies_to_market BOOLEAN NOT NULL, notional_max_applies_to_market BOOLEAN NOT NULL, notional_avg_price_mins INTEGER NOT NULL, captured_at_utc TIMESTAMP WITH TIME ZONE NOT NULL, reference_price_acquired_at_utc TIMESTAMP WITH TIME ZONE, PRIMARY KEY(run_id, symbol));;

-- paper_execution_rules_evidence (prospective Binance executionRules PRICE_RANGE evidence)
CREATE TABLE paper_execution_rules_evidence(run_id VARCHAR NOT NULL, symbol VARCHAR NOT NULL, contract_version VARCHAR NOT NULL, native_symbol VARCHAR, acquisition_status VARCHAR NOT NULL, price_range_present BOOLEAN, bid_limit_mult_up VARCHAR, bid_limit_mult_down VARCHAR, ask_limit_mult_up VARCHAR, ask_limit_mult_down VARCHAR, source_timestamp_utc TIMESTAMP WITH TIME ZONE, acquired_at_utc TIMESTAMP WITH TIME ZONE, PRIMARY KEY(run_id, symbol));;

-- paper_price_range_decisions (prospective local PRICE_RANGE decision reconstruction)
CREATE TABLE paper_price_range_decisions(run_id VARCHAR NOT NULL, decision_index INTEGER NOT NULL, symbol VARCHAR NOT NULL, side VARCHAR NOT NULL, idempotency_key VARCHAR NOT NULL, simulated_execution_price VARCHAR NOT NULL, reference_price_decimal VARCHAR, lower_bound_decimal VARCHAR, upper_bound_decimal VARCHAR, outcome VARCHAR NOT NULL, reason VARCHAR, decided_at_utc TIMESTAMP WITH TIME ZONE NOT NULL, PRIMARY KEY(run_id, decision_index), UNIQUE(run_id, idempotency_key));;
...
-- paper_runs
CREATE TABLE paper_runs(run_id VARCHAR PRIMARY KEY, started_at_utc TIMESTAMP WITH TIME ZONE NOT NULL, completed_at_utc TIMESTAMP WITH TIME ZONE, status VARCHAR NOT NULL, "mode" VARCHAR NOT NULL, official_scheduled BOOLEAN NOT NULL DEFAULT false, market_rule_evidence_required BOOLEAN NOT NULL DEFAULT false, execution_rules_evidence_required BOOLEAN NOT NULL DEFAULT false, schedule_key VARCHAR UNIQUE, attempted_schedule_key VARCHAR, signal_timestamp_utc TIMESTAMP WITH TIME ZONE, data_timestamp_utc TIMESTAMP WITH TIME ZONE, message VARCHAR, reconciliation JSON, market_rule_evidence_version INTEGER NOT NULL DEFAULT 1);;
