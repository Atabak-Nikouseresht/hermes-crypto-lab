...
-- paper_runs
CREATE TABLE paper_runs(run_id VARCHAR PRIMARY KEY, started_at_utc TIMESTAMP WITH TIME ZONE NOT NULL, completed_at_utc TIMESTAMP WITH TIME ZONE, status VARCHAR NOT NULL, "mode" VARCHAR NOT NULL, official_scheduled BOOLEAN NOT NULL DEFAULT false, market_rule_evidence_required BOOLEAN NOT NULL DEFAULT false, schedule_key VARCHAR UNIQUE, attempted_schedule_key VARCHAR, signal_timestamp_utc TIMESTAMP WITH TIME ZONE, data_timestamp_utc TIMESTAMP WITH TIME ZONE, message VARCHAR, reconciliation JSON, market_rule_evidence_version INTEGER NOT NULL DEFAULT 1);;
